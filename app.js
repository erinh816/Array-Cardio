...
for (i = 0; i < inventors.length; i++) {
  if (inventors[i].year < 1600 && inventors[i].year >= 1500) {
    console.table(inventors[i].first + " " + inventors[i].last);
  }
}

//answer - if want to still return an array
...
// Array.prototype.map()
// map takes in an array, does something and returns a new array of the same length
// 2. Give us an array of the inventors first and last names
let nameArray = [];
for (i = 0; i < inventors.length; i++) {
  let firstName = inventors[i].first;
  let lastName = inventors[i].last;
...
let total = 0;
for (i = 0; i < inventors.length; i++) {
  let difference = inventors[i].passed - inventors[i].year;
...
const oldest = inventors.sort(function (firstPerson, secondPerson) {
  if (
    firstPerson.passed - firstPerson.year >
